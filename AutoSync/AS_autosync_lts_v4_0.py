...
"""
AS_autosync_lts_v4_0.py

CAPS AutoSync LTS v4.0 (정식 운영 버전)
----------------------------------------
역할:
- CAPS Core가 만든 plan(caps_core_plan.json)을 읽고,
  필요한 모듈(VersionDocs, Scheduler, OmegaEngine, etc.)을
  우선순위에 따라 순서대로 실행한다.
- 각 모듈은 이미 LTS 기준으로 만들어진 개별 프로그램이며,
  AutoSync는 이들을 지휘하는 "오케스트라 지휘자" 역할을 한다.
- 패치/검증 후 SafeGuard LTS를 호출하여 GREEN/RED 상태를 확인하고,
  로그와 state 파일에 기록한다.

구조:
- config/autosync_config.json   → 기본 설정 (처음 실행 시 자동 생성)
- logs/autosync_log_YYYYMMDD.txt
- state/autosync_state.json
"""
...
class AutoSyncLTS:
    ...
    def _ensure_default_config(self) -> None:
        ...
        default_conf = {
            "version": "4.0",
            "description": "AutoSync LTS v4.0 기본 설정",
        ...
        if not dry_run:
            ...
            if sg_script and Path(sg_script).exists():
                ...
                try:
                    ...
                    # stdout에 state 정보가 포함되도록 SafeGuard를 만들었으니,
                    # 여기서는 단순히 returncode 기준으로만 GREEN/RED를 판단.
...
def main() -> None:
    ...
    if args.run:
        autosync.run_from_plan(dry_run=False)
        return
